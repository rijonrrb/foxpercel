                    ...
                    <div class="card">
                        ...
                        <div class="card-body py-5">
                            <div class="row d-flex justify-content-center">
                                <div class="col-xl-6">
                                    <form action="{{ route('admin.alarm.store') }}" method="post">
                                        @csrf
                                        <div class="form-group mb-3">
                                            ...
                                            <select name="level" id="level" class="form-control" required>
                                                ...
                                                @foreach ($data['defcon_levels'] as $defcon)
                                                    <option value="{{ $defcon->id }}">{{ $defcon->defcon_level }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="form-group mb-3">
                                            <label for="" class="form-label">Description <span class="text-danger">*</span></label>
                                            <textarea name="description" id="description" cols="30" rows="5" class="form-control" placeholder="Write description" required>{{old('description')}}</textarea>
                                        </div>
                                        <div class="form-group mb-3">
                                            <label for="" class="form-label">Action <span class="text-danger">*</span></label>
                                            <textarea name="action" id="action" cols="30" rows="5" class="form-control" placeholder="Write action" required>{{old('action')}}</textarea>
                                        </div>
                                        <div class="form-group mb-3">
